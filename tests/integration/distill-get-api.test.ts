...
const TEST_PREFIX = "test-distill-get-";
...
afterAll(async () => {
  await prisma.distillSource.deleteMany({ where: { job: { userId: { in: ["user-1", "user-2"] } } } });
  await prisma.distillJob.deleteMany({ where: { userId: { in: ["user-1", "user-2"] } } });
  await prisma.session.deleteMany({ where: { userId: { in: ["user-1", "user-2"] } } });
  await prisma.user.deleteMany({ where: { id: { in: ["user-1", "user-2"] } } });
